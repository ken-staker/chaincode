...
// ============================================================================================================================
// Get server_id - get a server asset from ledger
// ============================================================================================================================
func get_server(stub shim.ChaincodeStubInterface, id string) (Server, error) {
	var server Server
	serverAsBytes, err := stub.GetState(id)                  //getState retreives a key/value from the ledger
	if err != nil {                                          //this seems to always succeed, even if key didn't exist
		return server, errors.New("Failed to find server - " + id)
	}
	json.Unmarshal(serverAsBytes, &server)                   //un stringify it aka JSON.parse()

	if server.Id != id {                                     //test if server is actually here or just nil
		return server, errors.New("Server does not exist - " + id)
	}
